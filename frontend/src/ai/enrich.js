...
export function fieldFingerprint(deck, card) {
  return contentHash([
...
export function exampleFingerprint(deck, card) {
  return contentHash([
    deck?.title ?? '', deck?.description ?? '',
    card.spanish_text ?? '', card.english_text ?? '',
    ...examplePairs(card).flatMap((pair) => [pair?.es ?? '', pair?.en ?? '']),
  ].join('\u0000'));
}

export function clozeFingerprint(card) {
  const options = (Array.isArray(card.cloze_distractors_en) ? card.cloze_distractors_en : [])
    .map((option) => normalizeAnswer(String(option)))
    .sort();
  return contentHash([
    card.english_text ?? '',
    ...examplePairs(card).map((pair) => pair?.en ?? ''),
    ...options,
  ].join('\u0000'));
}
...
function auditFresh(card, key, fingerprint) {
  ...
  return Boolean(audit) && audit.status === 'pass' && audit.version === AUDIT_VERSIONS[key] &&
...
function applyLexical(card, response) {
  ...
  card.definition_en = optText(response.definition_en);
}

function applyEquivalents(card, response) {
  card.main_translations_es = normList(response.main_translations_es).slice(0, 3);
  card.collocations = normList(response.collocations).slice(0, 4);
}

function applySynonyms(card, response) {
  card.synonyms_en = normList(response.synonyms_en).slice(0, 3);
}
...
function normPair(pair) {
  ...
  const es = optText(pair.es ?? pair.example_es);
  const en = optText(pair.en ?? pair.example_en);
...
function applyClozeDistractors(card, response) {
  ...
  const sentences = examplePairs(card).map((pair) => pair?.en ?? '').filter(Boolean);
  card.cloze_distractors_en = normList(response.cloze_distractors_en)
...
async function solveSentence(card, sentenceEn, runPrompt) {
  ...
  const response = await runPrompt(clozeSolvePrompt(blanked, options));
  ...
  return {
    ...
    offenders: card.cloze_distractors_en.filter((option) => fitting.has(normalizeAnswer(option))),
  };
}
...
export async function processCard(draft, options = {}) {
  ...
  for (let round = 0; round < maxRounds; round += 1) {
    ...
    if (auditFields && !det.lexical.length && !det.equivalents.length && !det.synonyms.length &&
      ...
      const fingerprint = fieldFingerprint(deck, card);
      if (!auditFresh(card, 'field_quality', fingerprint)) {
        throwIfAborted();
        ...
        if (verdict.failingGroups.length > 0) {
          ...
          setAudit(card, 'field_quality', fingerprint);
          acted = true;
        }
      }
    }
    ...
    if (auditExamples && !det.examples.length && exampleAuditFails <= maxRepairs) {
      const fingerprint = exampleFingerprint(deck, card);
      if (!auditFresh(card, 'example_quality', fingerprint)) {
        ...
        if (failing.size) {
          ...
          setAudit(card, 'example_quality', fingerprint);
          acted = true;
        }
      }
    }
    ...
    if (auditCloze && wantCloze && !det.examples.length && !det.clozeDistractors.length &&
      ...
      const fingerprint = clozeFingerprint(card);
      if (!auditFresh(card, 'cloze_options', fingerprint)) {
        ...
        if (badPairs.size) {
          ...
          card.cloze_distractors_en = card.cloze_distractors_en.filter(
          ...
          setAudit(card, 'cloze_options', fingerprint);
          acted = true;
        }
      }
    }
